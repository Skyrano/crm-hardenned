...
@WebServlet("/gestionRoles")
public class GestionRole extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (utilisateur != null && (utilisateur.getRole() != null && utilisateur.getRole().getNom().equals("admin"))) {
            ...
            String listing = "" +
                    ...
                    "                <th scope='col' class='text-center'>Accès en écriture</th>\n" +
                    ...
                    "            <tbody>\n";
            for (Role r : roles) {
                String acces = r.getAccesEcriture() ? "Oui" : "Non";
                listing +=
                        "            <tr>\n" +
                                "                <th scope='row' class='text-center'>" + StringCleaner.cleaner(r.getNom(), 40) + "</th>\n" +
                                "                <th scope='row' class='text-center'>" + acces + "</th>\n" +
                                "                <th scope='row' class='text-center'>" + new RoleDAO().recupNbUtilisateursParRole(StringCleaner.cleaner(r.getNom(), 40)) + "</th>\n";
